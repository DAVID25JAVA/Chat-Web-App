...
function Sidebar({ selectedUser, setSelectedUser }) {
  return (
    <div className="bg-[#8185B2]/10 h-screen">
      {/* Header */}
      ...
      {/* User Data */}
      <div className="mt-5 overflow-y-auto h-[calc(100vh-180px)] scrollbar-hide">
        {userDummyData?.map((data) => (
          <div
            key={data?._id}
            onClick={() => setSelectedUser(data)}
            className={`flex gap-3 py-3 px-4 items-center cursor-pointer hover:bg-gray-800/50 rounded-full transition ${
